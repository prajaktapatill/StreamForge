...
resource "aws_iam_role" "jenkins_master_role" {
  ...
  # Trust Policy allowing EC2 instances to wear this identity
  assume_role_policy = jsonencode(local.jenkins_master_config)
}
...
resource "aws_iam_role_policy" "jenkins_master_policy" {
  ...
  policy = jsonencode(local.jenkins_master_config)
}
...
resource "aws_iam_role" "jenkins_agent_role" {
  ...
  assume_role_policy = jsonencode(local.jenkins_agent_config)
...
resource "aws_iam_role_policy" "jenkins_agent_policy" {
  ...
  policy = jsonencode(local.jenkins_agent_config)
}
...
resource "aws_iam_role_policy" "jenkins_agent_assume_master_policy" {
  ...
  role   = aws_iam_role.jenkins_agent_role.id
  policy = locals.jenkins_master_config
}
